...
test('updates timeline tracks releases newest-first with source links', async ({ page }) => {
  ...
  await expect(entries.first()).toContainText('0.33.5');
  await expect(entries.nth(1)).toContainText('0.33.4');
  await expect(entries.first()).toContainText('visible content');
  await expect(entries.first().locator('.latest-pill')).toHaveText('LATEST');
  await expect(entries.first().locator('.update-source')).toHaveAttribute('href', 'https://github.com/enzo-prism/midas/releases/tag/v0.33.5-midas.1');
...
test('download CTAs point to the actual Midas arm64 app ZIP', async ({ page }) => {
  ...
  for (const link of await links.all()) {
    await expect(link).toHaveAttribute('href', 'https://github.com/enzo-prism/midas/releases/download/v0.33.5-midas.1/Midas-0.33.5-macos-arm64.zip');
  }
