...
test('updates timeline tracks releases newest-first with source links', async ({ page }) => {
  ...
  await expect(entries.first()).toContainText('0.35.0');
  await expect(entries.nth(1)).toContainText('0.33.6');
  await expect(entries.first()).toContainText('Connect multiple Codex accounts');
  await expect(entries.first().locator('.latest-pill')).toHaveText('LATEST');
  await expect(entries.first().locator('.update-source')).toHaveAttribute('href', 'https://github.com/enzo-prism/midas/releases/tag/v0.35.0-midas.1');
...
test('download CTAs point to the actual Midas arm64 app ZIP', async ({ page }) => {
  ...
  for (const link of await links.all()) {
    await expect(link).toHaveAttribute('href', 'https://github.com/enzo-prism/midas/releases/download/v0.35.0-midas.1/Midas-0.35.0-macos-arm64.zip');
  }
